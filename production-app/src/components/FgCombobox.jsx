    ...
    <div className="overflow-hidden rounded-md border border-gray-300">
      {selectedLabel && (
        <div className="flex items-center justify-between gap-2 border-b border-gray-200 bg-etilog-light px-3 py-2">
          <span className="truncate text-[13px] font-semibold text-gray-900">{selectedLabel}</span>
          <button
            type="button"
            onClick={() => onChange(null)}
            className="shrink-0 text-[11px] font-medium text-etilog hover:underline"
          >
            Change
          </button>
        </div>
        ...
        <Command shouldFilter={false} className="flex flex-col">
          <div className="flex items-center gap-2 border-b border-gray-200 px-3">
            ...
            <Command.Input
              ...
              className="w-full bg-transparent py-2.5 text-[13px] outline-none placeholder:text-gray-400"
            />
          </div>
          ...
          <Command.List className="h-52 overflow-y-auto p-1">
            ...
            {results.map((product) => (
              <Command.Item
                ...
                className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-[13px] data-[selected=true]:bg-gray-100"
              >
                ...
                <span className="truncate text-[12px] text-gray-500">{product.description}</span>
              </Command.Item>
            ))}
            ...
            {!query.trim() && !results.length && !products.isFetching && (
              <div className={clsx('px-2 py-3 text-[12px] text-gray-400')}>
