...
### Roly a prístup k modulom

> **Opravené podľa auditu.** V portáli **neexistuje** matica „rola × modul" ani checkboxy v DB. Prístup je jeden **exkluzívny** string `users.role` (`user`, `spravca`, `sklad`, `sklad_read`, `admin`), **hardkódovaný** na dvoch miestach: `hasModuleAccess()` (frontend, kozmetika) a `requireDbRole(...)` per API route (server, skutočná ochrana). Jeden user = jedna rola.

Sú dve reálne cesty, treba sa rozhodnúť:

**(a) Podľa existujúceho vzoru (odporúčané pre MVP).** Pridať roly `vyroba` (úpravy) a `vyroba_read` (čítanie) — presne ako existujúci `sklad` / `sklad_read` split. Rozšíriť `hasModuleAccess()` o vetvu pre `production` a chrániť každý `/api/production/*` route cez `requireDbRole('admin','vyroba','vyroba_read')` (čítanie) resp. `requireDbRole('admin','vyroba')` (úpravy). Admin má prístup všade (už teraz `role === 'admin' → true`). Lacné, konzistentné, ihneď funguje. **Prístup skryť za jeden helper**, aby bol prechod na (b) neskôr lacný.
  - Obmedzenie: ostáva „1 user = 1 rola" (nedá sa byť súčasne `sklad` aj `vyroba`).

**(b) Skutočný model prístupu (portál-wide upgrade, neskôr).** Junction tabuľka `user_modules` / `role_permissions` + helper, ktorý čítajú frontend aj backend, + checkboxy v HR admin. Toto je pôvodne požadovaná matica a ruší limit exkluzivity — ale je to **zásah do zdieľaného auth celého portálu** (týka sa Sklad/Fleet/HR), teda väčší rozsah a riziko. Robiť samostatne, nie ako súčasť Production MVP.

**Autorizácia vždy na API/backend vrstve** — každý `/api/production/*` endpoint overuje rolu cez `requireDbRole`, klientský `hasModuleAccess` je len kozmetika.

> **Poznámka – externí partneri (napr. Kandrac):** „len svoja pobočka" sa rolou nevyjadrí; na to treba **location scope** ako atribút používateľa, ktorý Production API zohľadní pri filtrovaní. Nie je súčasť MVP.
...
**Users**
- Roly `vyroba` (úpravy) + `vyroba_read` (čítanie) podľa existujúceho `sklad`/`sklad_read` vzoru · `hasModuleAccess` + `requireDbRole` · admin default všade · autorizácia na API vrstve (viď §2, cesta a)
