...
### Roly a prístup k modulom (znovupoužiť existujúce role z HR modulu)

**Nevytvárať nové role pre výrobu.** Znovupoužiť **role, ktoré už existujú v HR module** v správe používateľov. Production modul iba **zaregistruje svoje capabilities** do existujúceho systému prístupov a číta, čo má daná rola/používateľ povolené.

Prístup k dlaždiciam/modulom sa rieši **per-rola cez checkboxy** v HR správe používateľov — presne tak, ako sa (bude) prepínať prístup k **Sklad** a **Vozový park**. Production pridá do tej istej matice dve nové položky:

| Modul / dlaždica | Capability | Význam |
|---|---|---|
| Sklad | `warehouse.access` | existujúce |
| Vozový park | `fleet.access` | existujúce |
| **Výrobný plán – čítanie** | `production.view` | vidí plán, história, notifikácie; žiadne úpravy |
| **Výrobný plán – úpravy** | `production.manage` | plánovanie, drag & drop, publish |

Matica prístupov (ilustračne — zapína/vypína sa checkboxmi pri role):

```
Rola                 Sklad  Vozový park  Výr. plán čítanie  Výr. plán úpravy
──────────────────────────────────────────────────────────────────────────
Admin                  ✔         ✔              ✔                  ✔      (default všetko)
Šéf výroby             ☐         ☐              ☑                  ☑
Pracovník výroby       ☐         ☐              ☑                  ☐
Skladník               ☑         ☐              ☐                  ☐
...                    ...       ...            ...                ...
...
Pravidlá:
- **Admin má default prístup všade** — implicitne, nezávisle od checkboxov (žiadne odškrtávanie po jednom).
- „Výrobný plán – úpravy" zahŕňa aj **publish**; ak by ste neskôr chceli publish oddeliť, pridá sa tretia capability `production.publish` do tej istej matice. Zatiaľ netreba.
- Production modul len **číta granted capabilities** aktuálneho používateľa a podľa nich gate-uje UI aj API. Nemá vlastnú tabuľku rolí.

**Autorizácia musí byť na API/backend vrstve**, nie iba schovaným buttonom v UI — každý `/api/production/*` endpoint overuje `production.view` / `production.manage`.

> **Poznámka – externí partneri (napr. Kandrac):** ak neskôr budete chcieť, aby externý partner videl **len svoju pobočku** (nie PO1/DS1/financie), samotné module-checkboxy to nevyjadria — na to treba **location scope** ako atribút používateľa/roly (`location_scope: [locationId, ...]`), ktorý Production API pri filtrovaní zohľadní. Nie je to súčasť MVP; capabilities architektúra to však umožní doplniť bez prerábania.
...
**Users**
- Znovupoužiť existujúce role z HR modulu · prístup cez module-access checkboxy (`production.view` / `production.manage`) · admin default všade · autorizácia na API vrstve
- (Location scope pre externých partnerov až neskôr — viď §2)
